...
## 0. 创建推演改为异步任务 (2026-07-16 已实施)

`POST /api/simulations` 不再同步等待，改为：
1. 立即返回 `task_id`
2. 前端轮询 `GET /api/simulations/create/{task_id}` 获取进度日志
3. 每个构建步骤的日志输出到前端 SystemLogs 面板

此改造解决了"创建推演时长黑盒、用户不知道进度"的痛点。
